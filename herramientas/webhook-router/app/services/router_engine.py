...
from app.handlers import HANDLERS_MAP, telegram_notifier, discord_notifier, slack_notifier
...
class AlertDeduplicator:
    ...
    async def check_and_update(self, event: Event) -> Tuple[bool, int]:
        """
        ...
        entry = await db_manager.get_deduplication_entry(key)
        ...
        if entry:
            ...
            if now - entry["first_seen"] < self.window_seconds:
                ...
                await db_manager.save_deduplication_entry(key, entry["first_seen"], now, new_count)
                ...
                await db_manager.save_deduplication_entry(key, now, now, 1)
                return False, 1
        else:
    ...
    async def clear_cache(self):
        ...
        await db_manager.clear_deduplication_cache()
...
class EventRouter:
    ...
    async def route_event(self, event: Event) -> Dict[str, any]:
        ...
        await db_manager.increment_event_counter(event.severity.value)

        # 2. Check if event is a duplicate in the persistent SQLite cache
        is_duplicate, duplicate_count = await self.deduplicator.check_and_update(event)
